...
exec {'drop-existing-project-user':
    ...
    command => 'psql -d postgres -c "DROP USER IF EXISTS edx_in_canvas"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-user':
    ...
    command => 'psql -d postgres -c "CREATE USER edx_in_canvas WITH PASSWORD \'edx_in_canvas\'"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'drop-project-db':
    ...
    command => 'psql -d postgres -c "DROP DATABASE IF EXISTS edx_in_canvas"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-db':
    ...
    command => 'psql -d postgres -c "CREATE DATABASE edx_in_canvas WITH OWNER edx_in_canvas"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
# Create a symlink from ~/edx_in_canvas to /vagrant as a convenience for the developer
file {'/home/vagrant/edx_in_canvas':
    ensure => link,
    target => '/vagrant',
}

# Create a virtualenv for edx_in_canvas
exec {'create-virtualenv':
    provider => 'shell',
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/edx_in_canvas'], Exec['known_hosts'], Exec['drop-project-db']],
    environment => ["HOME=/home/vagrant","WORKON_HOME=/home/vagrant/.virtualenvs"],
    command => '/vagrant/vagrant/venv_bootstrap.sh',
    creates => '/home/vagrant/.virtualenvs/edx_in_canvas',
}

# set the DJANGO_SETTINGS_MODULE environment variable
file_line {'add DJANGO_SETTINGS_MODULE env to postactivate':
    ensure => present,
    line => 'export DJANGO_SETTINGS_MODULE=edx_in_canvas.settings.local',
    path => '/home/vagrant/.virtualenvs/edx_in_canvas/bin/postactivate',
...
file_line {'add DJANGO_SETTINGS_MODULE env to postdeactivate':
    ...
    path => '/home/vagrant/.virtualenvs/edx_in_canvas/bin/postdeactivate',
    require => Exec['create-virtualenv'],
}
