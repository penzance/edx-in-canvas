...
# Create a symlink from ~/edx_lti_authoring to /vagrant as a convenience for the developer
file {'/home/vagrant/edx_lti_authoring':
    ensure => link,
    target => '/vagrant',
}

# Create a virtualenv for <project_name>
#require => [ Package['virtualenvwrapper'], File['/home/vagrant/edx_lti_authoring'], File['/etc/profile.d/oracle.sh'] ],
exec {'create-virtualenv':
    provider => 'shell',
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/edx_lti_authoring'] ],
    environment => ["HOME=/home/vagrant","WORKON_HOME=/home/vagrant/.virtualenvs"],
    command => '/vagrant/vagrant/venv_edx_authoring.sh',
    creates => '/home/vagrant/.virtualenvs/edx_lti_authoring',
}

# Active this virtualenv upon login
file {'/home/vagrant/.bash_profile':
    owner => 'vagrant',
    content => 'echo "Activating python virtual environment \"edx_lti_authoring\""; workon edx_lti_authoring',
